...
const SentimentTrend: React.FC<{ trend: number }> = ({ trend }) => {
  ...
  return (
    <div className="text-center">
      ...
      <div
        className={`flex items-center justify-center mt-1 font-bold text-xl ${colorClass}`}
      >
  ...
  return (
    ...
    >
      <Section title="상담 요약" icon={UserGroupIcon}>
        ...
        <MetricCard label="해결 상태" value={metrics.result_label} />
        <MetricCard
          label="문제 해결력"
          value={getSuggestionDescription(metrics.suggestions as number)}
        />
      ...
      <Section title="대화 흐름 분석" icon={ClockIcon}>
        <MetricCard
          label="상담사-고객 발화량"
          value={getTalkRatioDescription(metrics.talk_ratio as number)}
        />
        <MetricCard
          label="평균 응답 속도"
          value={metrics.avg_response_latency}
          unit="초"
        />
        <MetricCard
          label="대화 가로채기"
          value={metrics.interruption_count}
          unit="회"
        />
        {/* 침묵 비율은 100을 곱해서 %로 표시 */}
        <MetricCard
          label="침묵 비율"
          value={(metrics.silence_ratio as number) * 100}
          unit="%"
        />
      </Section>
