...
async function startReplicatePrediction(fileUrl: string) {
  ...
  if (process.env.NODE_ENV === "production") {
    if (!webhookUrl.startsWith("https://")) {
      throw new Error(
        `Invalid webhook URL in production: ${webhookUrl}. Must be HTTPS.`
      );
    }
  } else if (!webhookUrl.startsWith("http://localhost")) {
    throw new Error(
      `Invalid webhook URL: ${webhookUrl}. Must be HTTPS or localhost.`
    );
  }
  ...
  const response = await fetch(
    "https://api.replicate.com/v1/deployments/ohdurma/feple-ai-v3/predictions",
    {
      method: "POST",
      headers: {
